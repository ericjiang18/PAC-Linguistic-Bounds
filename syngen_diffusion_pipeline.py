...
from compute_loss import (
    ...
    compute_total_loss  # Import the new loss function
)
...
class SynGenDiffusionPipeline(StableDiffusionPipeline):
    def __init__(self,
                 ...
                 lambda_pac: float = 0.001,  # New hyperparameters
        ...
        self.lambda_pac = lambda_pac  # Initialize the new hyperparameters
    ...
    def _syngen_step(
        ...
        with torch.enable_grad():
            ...
            for latent, text_embedding in zip(latents, text_embeddings):
                ...
                # Run the UNet and get the noise prediction
                noise_pred = self.unet(
                    latent,
                    t,
                ...
                # Compute the loss using the updated compute_total_loss function
                loss = self._compute_loss(
                    attention_maps=attention_maps,
                    prompt=prompt,
                    model=self.unet  # Pass the model for PAC-Bayes regularization
                )
    ...
    def _compute_loss(
            self, attention_maps: List[torch.Tensor], prompt: Union[str, List[str]], model
    ) -> torch.Tensor:
        attn_map_idx_to_wp = get_attention_map_index_to_wordpiece(self.tokenizer, prompt)
        loss = self._attribution_loss(attention_maps, prompt, attn_map_idx_to_wp, model)
        return loss
    ...
    def _attribution_loss(
        ...
        # Compute the total loss using compute_total_loss from compute_loss.py
        total_loss = compute_total_loss(
            attention_maps=attention_maps,
            prompt=prompt,
            attn_map_idx_to_wp=attn_map_idx_to_wp,
            model=model,
            lambda_pac=self.lambda_pac,
